...
class Regression1944Test extends TestCase {
    ...
    public function testImpossibleCollectionChange() {
        ...
        $this->type("id=PersonAuthorFirstName_1", "tester");
        $this->type("id=PersonAuthorLastName_1", "test");
        $this->type("id=TitleMain_1", "Entenhausen");
        $this->type("id=TitleAbstract_1", "drltk-jzrkld5zhjkls");
        $this->select("id=Institute_1", "label=Technische Universität Hamburg-Harburg");
        $this->click("id=browseDownInstitute");
        $this->waitForPageToLoad();
        $this->click("id=browseDownInstitute");
        $this->waitForPageToLoad();
        $this->select("id=SubjectMSC_1", "label=03-XX MATHEMATICAL LOGIC AND FOUNDATIONS");
        ...
        $this->waitForPageToLoad();
        $this->select("id=SubjectMSC_1", "label=22-XX TOPOLOGICAL GROUPS, LIE GROUPS (For transformation groups, see 54H15, 57Sxx, 58-XX. For abstract harmonic analysis, see 43-XX)");
        $this->click("id=browseDownSubjectMSC");
        $this->waitForPageToLoad();
